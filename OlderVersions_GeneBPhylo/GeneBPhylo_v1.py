...
###################################################################################################
        ...
        Clade_Duplications = {}
        for i in range(0,len(Tree_Path)):
            if Added in Tree_Path[i]:
                Clade_Duplications[Tree_Path[i].replace(Added, '')] = Duplications_by_Clade[i]
            else:
...
###################################################################################################
        ...
        for key, value in Clade_Duplications.items():
            if Specie_Name in key:
                writer.writerow([key, list(Clade_Path.keys())[counter], len(value),Duplication_Events_Ref_Spe, Branch_Length.get(key + Added),(Duplication_Events_Ref_Spe)/(Branch_Length.get(key + Added)*1000)])  
            if key not in Branch_Length.keys() and Specie_Name not in key:
                writer.writerow([key, list(Clade_Path.keys())[counter],len(value),Duplication_events[counter], 0,0])
